...
if (!segmentKey) {
  console.warn('dev-sandbox-console-plugin: has no SEGMENT_KEY configureed.');
}
...
const initSegment = () => {
  ...
  if (analytics.invoked) {
    console.error('dev-sandbox-console-plugin: segment snippet included twice.');
  } else {
...
export const eventListener = async (eventType: string, properties?: any) => {
  if (!segmentKey) {
    console.log('dev-sandbox-console-plugin: received telemetry event: ', eventType, properties);
  } else {
    switch (eventType) {
      case 'identify':
        {
