...
1. Build the image:
   ```sh
   docker build -f Dockerfile.plugins.demo -t quay.io/$USER/dev-sandbox-console-plugin .
   ```
2. Run the image:
   ```sh
   docker run -it -p 9001:9001 quay.io/$USER/dev-sandbox-console-plugin
   ```
3. Push the image to image registry:
   ```sh
   docker push quay.io/$USER/dev-sandbox-console-plugin
   ```
