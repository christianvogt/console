...
import TopologyShortcuts from '../topology/TopologyShortcuts';
import { LAST_TOPOLOGY_VIEW_LOCAL_STORAGE_KEY } from '../topology/components/const';
import { TOPOLOGY_SEARCH_FILTER_KEY } from '../topology/filters';
...
export const TopologyPage: React.FC<TopologyPageProps> = ({ match }) => {
  ...
  if (!showGraphView && !showListView) {
    return (
      <Redirect
        to={`/topology3/${namespace ? `ns/${namespace}` : 'all-namespaces'}/${
